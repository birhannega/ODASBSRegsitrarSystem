...
<div
		class=" container-fluid col-md-12  col-lg-12 col-sm-12 col-xs-12 ">
				...
				<div class="panel panel-heading "><strong>Teacher resignation
				 form</strong></div>
...
<form action="${pageContext.request.contextPath }/Resign"method="post"class="form-inline">
<%
...
if(request.getParameter("teacherid")==null||request.getParameter("teacherid")==""){
	%>
...
ResultSet rsstuwithdraw=studentwithdraw.executeQuery("select *from TBL_teacher where TeacherId='"+teacherId+"'and Branch='"+branch+"' and Status='Active'");
if(rsstuwithdraw.next()){
	teacherid=rsstuwithdraw.getString("TeacherId");
...
<tr><td colspan="5" class="active"> <p class="text-info">Profile of Teacher to be Resigned</p> </td></tr>
<tr>
<th>Teacher ID</th>
<th>FullName</th>
<th>Gender</th>
...
<tr>
<td><%=teacherid %><input type="hidden"value="<%=teacherid %>"name="teacherid"></td>
<td><%=fname+"  "+lname %></td>
<td><%=gender %></td>
<td><%=age %></td>
<td><input type="submit" class="btn btn-danger" id="resign"value="submit"></td>
					</table>
					</div>
				</form>	
					<%}
else{ %><strong style="color:red;"> ID did not match to any Teacher</strong>
					
					
					<%}}
Statement stat_session=dbcon.getConnection().createStatement();
ResultSet rs_session=stat_session.executeQuery("select currentState from TBL_users where userName='"+session.getAttribute("registrar").toString()+"' ");
if(rs_session.next())
{
