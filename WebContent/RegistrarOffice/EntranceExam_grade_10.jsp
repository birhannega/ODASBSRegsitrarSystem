...
<%String userid=session.getAttribute("registrar").toString(),branch=null;
	...
	<div class="panel panel-primary">
			<div class="panel-heading">
				<h4 class="text-uppercase">ODA Special Boarding School
					<%=branch%>
		...
		<p class="alert alert-info"><strong>${studentid} ${inserted} ${notinserted}</strong> </p>
	  ...
	  <table class="table table-responsive table-bordered">				   
		...
		ResultSet entranceexam=entrance.executeQuery("select *from TBL_student where Grade='10'and Branch='"+branch+"'");
		while(entranceexam.next()){
		   ...
		   <tr><form action="${pageContext.request.contextPath}/Grade_10_exam"method="post"onsubmit="return chekForm(this)">
		...
		<td><%=firstname %></td>
		<td><%=grade %>
		<input type="hidden"value="<%=grade %>"name="grade"></td>
		...
		while(rssubject.next()){
			...
			String name=subjectname+i;
					%>
			<td>
			<input type="text" name="<%=id%>" class="form-control" required="required" min="1" max="100">
			<input type="hidden" name="<%=name%>" value="<%=rssubject.getString(1)%>" class="form-control" 
