...
{
	%>
	<div class=" well well-sm alert-warning"><h5 class="text-danger">choose test mechanism</h5></div>
<%}
else
{
...
while(Result.next())
{
	...
	%>
	<tr>
	<form method="post" action="${pageContext.request.contextPath}/SaveTestResult" id="mark" name="mark" onsubmit="return valiatemark(this)">
	<td>
	<%=request.getParameter("test")%>
	<input name="teacherid" type="hidden" value="<%=TID%>">
	<input name="subid" type="hidden" value="<%=subid%>">
	<input name="studid" type="hidden" value="<%=id%>">
	<input name="branch" type="hidden" value="<%=branch%>">
	<input name="test" type="hidden" value="<%=request.getParameter("test")%>">	
	...
	<td><%= fname+" "+lname%></td>
	<td><input type="number" step="0.01" class="form-control" name="obtainedMark" style="width:150px" required="required" min="<%=min %>" max="<%=max%>"></td>
	<td><button type="submit" class="btn btn-primary fa fa-save" value="save mark"></button>
